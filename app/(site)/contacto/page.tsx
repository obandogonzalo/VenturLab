      ...
      <section className="py-24 px-6" style={{ background: "var(--bg)" }}>
        <div className="max-w-7xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20, filter: "blur(6px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}>
            <p className="label-tag mb-6">Contacto</p>
            <h1 className="heading-xl mb-6 max-w-3xl" style={{ color: "var(--text-1)" }}>
              Hablemos de<br />
              <span style={{ color: "var(--accent)" }}>tu proyecto.</span>
            </h1>
      ...
      <section className="py-20 px-6" style={{ background: "var(--bg-2)" }}>
        <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-16">
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.55, delay: 0.1 }} className="flex flex-col gap-5">
            ...
            <a href="https://wa.me/56900000000" target="_blank" rel="noopener noreferrer" className="card flex items-start gap-4 p-6 group">
